...
/** 관리 기능(인원관리 등) 접근 — SUPER_ADMIN 전용. "관리자(ADMIN)"는 열람만 가능하고 관리 기능은 없습니다. */
export async function requireSuperAdmin(req: NextRequest) {
  const { uid, profile } = await requireUser(req);
  if (profile.org_role !== "SUPER_ADMIN") {
    throw new ApiAuthError("슈퍼 관리자만 수행할 수 있는 작업입니다.", 403);
  }
  return { uid, profile };
}

/** meetings/{meetingId}/permissions/{uid} 문서를 확인해 회의 접근 권한을 검사합니다.
 * SUPER_ADMIN / ADMIN은 permissions 문서가 없어도 전체 컨텐츠 열람 권한으로 통과합니다. */
export async function requireMeetingRole(
  ...
  const { uid, profile } = await requireUser(req);
  if (profile.org_role === "SUPER_ADMIN" || profile.org_role === "ADMIN") {
    return { uid, profile, role: "AUTHOR" as const };
  }
