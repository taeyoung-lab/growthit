...
import { grantMeetingAccess } from "@/lib/firebase/meetingAccess";
...
export async function POST(req: NextRequest, { params }: { params: Record<string, never> }) {
  try {
    ...
    // 공유 대상에게 즉시 회의 접근 권한 부여 (permissions 서브컬렉션) + member_uids 동기화
    // (Cloud Functions를 쓰지 않으므로 이 헬퍼가 두 쓰기를 함께 처리합니다 — meetingAccess.ts 참고)
    ...
    // 서버(API 라우트)에서만 쓰이는 값이라 브라우저에 노출할 필요가 없어 NEXT_PUBLIC_ 접두사를 붙이지 않습니다.
